...
class Invoice:
    ...
    def __init__(self):
        ...
        pdfmetrics.registerFont(
            TTFont("DejaVu", "/usr/share/fonts/truetype/ttf-dejavu/DejaVuSans.ttf")
        )
...
Číslo účtu: %s
Variabilní symbol: %s"""
            % (self.provider.bank_name, self.provider.bank_account, self.vs),
        )
        self.pdf.drawText(text)
    ...
    def drawDates(self, TOP, LEFT):
        ...
        self.pdf.drawString(
            (LEFT) * mm, (TOP + 1) * mm, "Datum vystavení: %s" % today.strftime("%d.%m.%Y")
        )
        self.pdf.drawString(
            (LEFT) * mm, (TOP - 4) * mm, "Datum splatnosti: %s" % payback.strftime("%d.%m.%Y")
        )
