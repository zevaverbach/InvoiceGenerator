...
def prepare_invoice_draw(self):
    ...
    if self.invoice.currency:
        warnings.warn(
            "currency attribute is deprecated, use currency_locale instead", DeprecationWarning
        )
...
class SimpleInvoice(BaseInvoice):
    ...
    def _drawItems(self, TOP, LEFT):  # noqa
            ...
            if items_are_with_tax:
                if float(int(item.count)) == item.count:
                    self.pdf.drawRightString(
                        (LEFT + 85) * mm,
                        (TOP - i) * mm,
                        u"%s %s" % (locale.format("%i", item.count, grouping=True), item.unit),
                    )
                else:
                    self.pdf.drawRightString(
                        (LEFT + 85) * mm,
                        (TOP - i) * mm,
                        u"%s %s" % (locale.format("%.2f", item.count, grouping=True), item.unit),
                    )
                self.pdf.drawRightString(
                    (LEFT + 110) * mm,
                    (TOP - i) * mm,
                    currency(item.price, self.invoice.currency, self.invoice.currency_locale),
                )
                self.pdf.drawRightString(
                    (LEFT + 134) * mm,
                    (TOP - i) * mm,
                    currency(item.total, self.invoice.currency, self.invoice.currency_locale),
                )
                self.pdf.drawRightString((LEFT + 144) * mm, (TOP - i) * mm, "%.0f %%" % item.tax)
                self.pdf.drawRightString(
                    (LEFT + 173) * mm,
                    (TOP - i) * mm,
                    currency(item.total_tax, self.invoice.currency, self.invoice.currency_locale),
                )
                i += 5
            else:
                if float(int(item.count)) == item.count:
        ...
        if will_wrap:
            self.pdf.rect(
                LEFT * mm, (TOP - i) * mm, (LEFT + 156) * mm, (i + 2) * mm, stroke=True, fill=False
            )  # 140,142
            self.pdf.showPage()
        ...
        if self.invoice.rounding_result:
            ...
            self.pdf.drawString(
                ...
                (TOP - i) * mm,
                currency(
                    self.invoice.difference_in_rounding,
        ...
        if not items_are_with_tax:
            ...
            self.pdf.drawString(
                ...
                % (
                    _(u"Total"),
                    currency(
                        self.invoice.price, self.invoice.currency, self.invoice.currency_locale
                    ),
                ),
            )
        else:
            ...
            self.pdf.drawString(
                ...
                % (
                    _(u"Total with tax"),
                    currency(
                        self.invoice.price_tax, self.invoice.currency, self.invoice.currency_locale
                    ),
                ),
            )
    ...
    def _drawDates(self, TOP, LEFT):
        ...
        if self.invoice.date and self.invoice.use_tax:
            items.append(
                (
                    ...
                    "%s: %s"
                    % (
                        _(u"Date of exposure taxable invoice"),
                        format_date(self.invoice.date, locale=lang),
                    ),
                )
            )
            ...
            items.append(
                (
                    LEFT * mm,
                    "%s: %s"
                    % (_(u"Date of exposure"), format_date(self.invoice.date, locale=lang)),
                )
            )
        if self.invoice.payback:
            items.append(
                (
                    LEFT * mm,
                    "%s: %s" % (_(u"Due date"), format_date(self.invoice.payback, locale=lang)),
                )
            )
        if self.invoice.taxable_date:
            items.append(
                (
                    LEFT * mm,
                    "%s: %s"
                    % (_(u"Taxable date"), format_date(self.invoice.taxable_date, locale=lang)),
                )
            )
...
class CorrectingInvoice(SimpleInvoice):
    ...
    def drawCorretion(self, TOP, LEFT):
        self.pdf.setFont("DejaVu", 8)
        self.pdf.drawString(
            LEFT * mm, TOP * mm, _(u"Correction document for invoice: %s") % self.invoice.number
        )
        self.pdf.drawString(
            LEFT * mm, (TOP - 4) * mm, _(u"Reason to correction: %s") % self.invoice.reason
        )
